...
(function (global, factory) {
    ...
    (global = global || self, global.w6sCordovaImport = factory());
    ...
    var isValidHost = function isValidHost(uri) {
      if (!lodash_isstring(uri)) return false;
    ...
    function () {
      function CordovaImportLib(host) {
        ...
        if (host) {
          this.config = lodash_merge(config, {
            SDK_HOST: host
          });
        } else {
          this.config = config;
      ...
      _createClass(CordovaImportLib, [{
        ...
        value: function getImportUri() {
          ...
          invariant_1(platform, 'Please open in the correct webview');
    ...
    function cordovaImportInit(host) {
      try {
        if (host) {
          invariant_1(isValidHost(host), 'Please enter a valid host, such as //workplus.io/');
        }
